...
func migrateConfiguration(oldCfg Configuration) static.Configuration {
	if oldCfg.Retry != nil {
		fmt.Println("Retry must be converted manually. Look https://docs.traefik.io/v2.0/middlewares/retry/")
	}

	if oldCfg.Constraints != nil {
		fmt.Println("Global Constraints must be converted manually to provider constraints. Look https://docs.traefik.io/v2.0/providers/docker/#constraints")
	}

	if oldCfg.Web != nil {
		fmt.Println("Web must be converted manually. Look https://docs.traefik.io/v2.0/operations/api/")
	}
...
func migrateAPI(oldCfg Configuration) *static.API {
	...
	if oldCfg.API.EntryPoint != "" {
		fmt.Printf("The entry point (%s) defined in API must be converted manually. Look https://docs.traefik.io/v2.0/operations/api/\n", oldCfg.API.EntryPoint)
	}
...
func migrateEntryPoints(oldCfg Configuration) static.EntryPoints {
	...
	for name, entryPoint := range *oldCfg.EntryPoints {
		if entryPoint.Compress {
			fmt.Printf("Compress on entry point %q must be converted manually. Look https://docs.traefik.io/v2.0/middlewares/compress/\n", name)
		}
		if entryPoint.TLS != nil {
			fmt.Printf("TLS on entry point %q must be converted manually. Look https://docs.traefik.io/v2.0/routing/routers/#tls\n", name)
		}
		if entryPoint.Redirect != nil {
			fmt.Printf("Redirect on entry point %q must be converted manually. Look https://docs.traefik.io/v2.0/middlewares/redirectscheme/\n", name)
		}
		if entryPoint.WhiteList != nil {
			fmt.Printf("WhiteList on entry point %q must be converted manually. Look https://docs.traefik.io/v2.0/middlewares/ipwhitelist/\n", name)
		}
		if len(entryPoint.WhitelistSourceRange) != 0 {
			fmt.Printf("WhitelistSourceRange on entry point %q must be converted manually. Look https://docs.traefik.io/v2.0/middlewares/ipwhitelist/\n", name)
		}
