...
func migrateMarathon(oldCfg Configuration) *marathon.Provider {
	...
	if len(oldCfg.Marathon.Domain) != 0 {
		fmt.Printf("The domain (%s) defined the Marathon provider must be converted manually. Look https://docs.traefik.io/v2.0/providers/marathon/#defaultrule\n", oldCfg.Marathon.Domain)
	}
...
func migrateDocker(oldCfg Configuration) *docker.Provider {
	...
	if len(oldCfg.Docker.Constraints) != 0 {
		fmt.Println("The constraints defined in the Docker provider must be converted manually. Look https://docs.traefik.io/v2.0/providers/docker/#constraints")
	}

	if len(oldCfg.Docker.Domain) != 0 {
		fmt.Printf("The domain (%s) defined in the Docker provider must be converted manually. Look https://docs.traefik.io/v2.0/providers/docker/#defaultrule\n", oldCfg.Docker.Domain)
	}
...
func migrateRest(oldCfg Configuration) *rest.Provider {
	...
	if oldCfg.Rest.EntryPoint != "" {
		fmt.Printf("The entry point (%s) defined in the REST provider must be converted manually. Look https://docs.traefik.io/v2.0/operations/api/\n", oldCfg.Rest.EntryPoint)
	}
